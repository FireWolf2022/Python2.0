...
class Cliente:
    ...
    """
    Falta agregar la opcion de VIP a cada accion y mostrarla para el usuario vip, y pedir una contraseña para el usuario admin
    pa que sea mas realista, por lo demas creo que esta bien
    """
    ...
    def transferir(self):
        ...
        try:
            ...
            b = int(input(f"Ingrese la cantidad que desea transferir, dispone de {cuentas[self.balance]}: "))
            if b <= self.balance:
                if a == 11111:
                    if not self.vip:
                        ...
                        vips[self.num].append("Donacion realizada")
                        cuentas['admin'] += b
                        self.balance -= b
